...
\begin{table}[]
    ...
    \caption{Means and standard deviations for the statistics of the two populations. Only includes features with a non-nan F1-score and a nonzero density. According to a two-sample bootstrap test, the distribution means for all statistics are different with $p<0.0001$.}
...
This raises a question of whether these clusters are also meaningful directly in terms of the feature directions (\autoref{sec:feature_directions})?
...
This can be seen in \autoref{tab:directions} where the high density cluster has a mean cosine similarity of $0.96$, and the mean distance of a direction in the normalized cluster to the mean direction of that cluster is $0.15$.
Meanwhile, the other clusters have a mean cosine similarity less than $0.02$ and a mean distance to mean direction of at least $0.99$.
...
\begin{table}[]
    ...
    \caption{Results of statistical analysis on the directions (see \autoref{sec:feature_directions}) of various feature clusters.
    ...
    \texttt{Mean dist.} is the mean distance between directions in the cluster to the mean cluster direction.
    \texttt{PCA} and \texttt{Mean dist.} are split into raw and normalized, where the former is the value for the directions taken directly from the SAE while for the latter the directions have been normalized to norm $1$.
    }
